...
class StockAgent:
    def __init__(self, auth, config_loader, agent_settings_path="config/agent_settings.yaml"):
        ...
        # Claude Client
        api_key = secrets.get("ANTHROPIC_API_KEY") or None
        if not api_key:
            print("[StockAgent] Warning: ANTHROPIC_API_KEY not found in secrets.yaml, falling back to env")
    ...
    def collect_data(self, stock_code):
        ...
        # 2. Econ (Calendar)
        # Simplified: fetch upcoming events
        events = self.econ.fetch_all()
        # Filter for relevant/upcoming
        data['econ'] = events[:10] 
...
"""
        ...
        try:
            ...
            return f"에이전트 분석 중 오류가 발생했습니다: {str(e)}"
...
if __name__ == "__main__":
    ...
    loader = ConfigLoader()
    # Note: KIS credentials needed for SupplyAnalyzer part
    # This example assumes REAL or PAPER config is available
    try:
